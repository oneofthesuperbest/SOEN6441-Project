...
/**
 * An observer class that waits for notifications from
 * the the LogEntryBuffer class and writes its contents in a text file.
 * In its current state, all the logs are saved into the res/logs/logs.txt
 * file.
 */
public class FileEntryLogger implements Observer {

    /**
     * Constructor for the FileEntryLogger Class. Attaches `this` instance
     * to a LogEntryBuffer object.
     *
     * @param p_logEntryBuffer Reference to the LogEntryBuffer object
    ...
    /**
     * Write the current log entry to file after the view has been
     * notified of a state change in the LogEntryBuffer model.
     *
