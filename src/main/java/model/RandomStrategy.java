...
public class RandomStrategy extends Strategy {
	...
	public int issueOrder() {
		if (d_remainingReinforcements == -1) {
			...
			d_maxnumberOfOrder = 5;
			for (CountryModel l_playerCountry : d_player.getOwnedCountry()) {
				l_playerCountry.setPotentialArmies(l_playerCountry.getArmies());
			}
		}
		if (d_player.getOwnedCountry().size() > 0) {
			if (d_remainingReinforcements > 0) {
				int l_randomCountry = (int) Math.random() * d_player.getOwnedCountry().size();
				d_gameEngine.getPhase().delop(new DeployOrder(d_player.getOwnedCountry().get(l_randomCountry).getName(),
						d_remainingReinforcements, d_player, d_gameEngine));
				d_player.getOwnedCountry().get(l_randomCountry)
						.setPotentialArmies((d_player.getOwnedCountry().get(l_randomCountry).getPotentialArmies()
								+ d_remainingReinforcements));
				...
				if (l_randomAction == 0 || !d_player.containsCard(2)) {
					...
					ArrayList<CountryModel> l_neighbors = l_mapController
							.getNeighbors(d_player.getOwnedCountry().get(l_randomCountry));
					int l_armiesToMove = d_player.getOwnedCountry().get(l_randomCountry).getArmies() > 0
							? d_player.getOwnedCountry().get(l_randomCountry).getArmies()
							: d_player.getOwnedCountry().get(l_randomCountry).getPotentialArmies();
					d_gameEngine.getPhase()
							.advance(new AdvanceOrder(d_player.getOwnedCountry().get(l_randomCountry).getName(),
									l_neighbors.get(0).getName(), l_armiesToMove, d_player, d_gameEngine));
				} else {
					int l_randomCountry = (int) Math.random() * d_player.getOwnedCountry().size();
					int l_randomTargetCountry = (int) Math.random()
							* d_gameEngine.getMapState().getListOfCountries().size();
					int l_armiesToMove = d_player.getOwnedCountry().get(l_randomCountry).getArmies() > 0
							? d_player.getOwnedCountry().get(l_randomCountry).getArmies()
							: d_player.getOwnedCountry().get(l_randomCountry).getPotentialArmies();
					d_gameEngine.getPhase()
							.airlift(new AirliftOrder(
									d_player.getOwnedCountry().get(l_randomCountry).getName(), d_gameEngine
											.getMapState().getListOfCountries().get(l_randomTargetCountry).getName(),
									l_armiesToMove, d_player, d_gameEngine));
					d_player.hasCard(2);
				}
				d_maxnumberOfOrder--;
				return 1;
